...
if [[ ! $response =~ cipherstash_proxy_statements_execution_duration_seconds\{.*quantile=\"1\"\} ]]; then
    echo "error: did not see execution duration metric with quantile=\"1\" in output"
    exit 1
fi

if [[ ! $response =~ cipherstash_proxy_statements_session_duration_seconds\{.*quantile=\"1\"\} ]]; then
    echo "error: did not see session duration metric with quantile=\"1\" in output"
    exit 1
fi
